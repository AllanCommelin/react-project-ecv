...
// Actions
const { addArticle, addArticles, removeArticle, addOrUpdateArticle, findAndUpdateArticle } =
  articles.actions
...
export const retrieveArticlesByTitle = (title) => async (dispatch) => {
  try {
    const response = await fetch(`${process.env.REACT_APP_API_URL}articles?title_like=${title}`, {
      method: 'GET',
    })
    const data = await response.json()
    dispatch(addArticles(data))
...
export const getArticle = (state, id) =>
  state.articles.list.find((article) => article.id === parseInt(id))

const getArticleByIndex = (state, index) => state.articles.list[index]

export const getArticleIndex = (state, id) =>
  state.articles.list.findIndex((article) => article.id === id)
